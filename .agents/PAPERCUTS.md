...
- [ ] `2026-09-10T07:41:11Z` — `claude` — `npx vitest run` intermittently fails `src/server/mcp/oauth-provider.test.ts` with "Test timed out in 5000ms", but the file passes alone in ~1.6s. The test does `await import("./oauth-provider")` inside the test body, so it pays module transform cost while the other 145 files are transforming in parallel. Either statically import it (the repo's own testing rule) or give that file a longer testTimeout.
