...
// Adapt one Deep Insights tool into an AI SDK tool. The shared handler receives the
 ...
 * free site-reading tools. Every tool the Deep Insights MCP server exposes is
...
export function buildSamMcpTools(
  ...
  return {
    ...
    get_product_info: tool({
      description:
        "The Deep Insights fact sheet: what the product does, plans/pricing, credit costs, integrations, MCP setup. Call before answering questions about Deep Insights itself. Uses no credits.",
      inputSchema: z.object({}),
